...
n = 10 # size of matrix
A = np.array(MatrixGeneration.GENERATE(n)) # generate the matrix
...
f = open("stats_outputs_numpy.txt", 'w')
...
def engine(l: float, r: float, Delta_P: float, Nu: float):
    ...
    for i in range(1, n + 1):
        Absolute_error.append(abs(x[i-1] - EV[i]))
